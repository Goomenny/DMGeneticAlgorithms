...
def I1(x):
    0.05*(x-1)**2+(3-2.9*exp(-2.77257*x**2))*(1-cos(x*(4-50*exp(-2.77257*x**2))))
def I2(x):
    1-0.5*cos(1.5*(10*x-0.3))*cos(31.4*x)+0.5*cos(5**0.5*10*x)*cos(35*x)]
def I3(x):
    x,y = x
    0.1*x**2+0.1*y**2-4*cos(0.8*x)-4*cos(0.8*y)+8
def I4(x):
    x,y=x
    (0.1*1.5*y)**2+(0.1*0.8*x)**2-4*cos(0.8*1.5*y)-4*cos(0.8*0.8*x)+8
def I5(x):
    x,y=x
    100*(y-x**2)**2+(1-x)**2
def I6(x):
    x,y=x
    -10/(0.005*(x**2+y**2)-cos(x)*cos(y/2**0.5)+2)+10
def I7(x):
    x,y=x
    -100/(100*(x**2-y)+(1-x)**2+1)+100
def I8(x):
    x,y=x
    (1-sin((x**2+y**2)**0.5)**2)/(1+0.001*(x**2+y**2))
def I9(x):
    x1,x2 = x
    0.5*(x1**2+x2**2)*(2*0.8+0.8*cos(1.5*x1)*cos(3.14*x2)+0.8*cos(5**0.5*x1)*cos(3.5*x2))
def I10(x):
    return I9(x)
def I11(x):
    x1,x2 =x
    x1**2*fabs(sin(2*x1))+x2**2*fabs(sin(2*x2))-1/(5*x1**2+5*x2**2+0.2)+5
def I12(x):
    x1,x2=x
    0.5*(x1**2+x1*x2+x2**2)*(1+0.5*cos(1.5*x1)*cos(3.2*x1*x2)*cos(3.14*x2)+0.5*cos(2.2*x1)*cos(4.8*x1*x2)*cos(3.5*x2))
